            ...
            <div class="card sf-card mb-4">
                <div class="card-body p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        <div>
                            <span class="badge bg-light text-dark mb-2">{{ $list->sponsorshipProgram->program_name }}</span>
                    ...
                    @if (in_array($list->status, [\App\Enums\FixedListStatus::Draft, \App\Enums\FixedListStatus::Rejected], true))
                        <div class="border-top pt-3 mt-3">
                            ...
                            <form method="POST" action="{{ route('fassg.fixed-lists.items.store', $list) }}"
                                class="row g-2">
                                @csrf
                                <div class="col-md-3"><input type="text" name="student_name"
                                        class="form-control form-control-sm" placeholder="Full Name" required></div>
                                <div class="col-md-3"><input type="text" name="student_id_number"
                                        class="form-control form-control-sm" placeholder="Student ID" required></div>
                                <div class="col-md-3"><input type="text" name="course"
                                        class="form-control form-control-sm" placeholder="Course" required></div>
                                <div class="col-md-2"><input type="number" name="year_level"
                                        class="form-control form-control-sm" placeholder="Year" min="1"
                                        max="5" required></div>
                                <div class="col-md-1"><button type="submit"
                                        class="btn btn-sf-navy btn-sm w-100">Add</button></div>
                            </form>
                        </div>
                    @endif
                </div>
                        ...
                        <tbody>
                            @forelse ($list->items as $item)
                                <tr>
                                    ...
                                    <td><span
                                            class="badge {{ $item->is_sle_fhe_verified ? 'bg-success-subtle text-success-emphasis' : 'bg-warning-subtle text-warning-emphasis' }}">{{ $item->is_sle_fhe_verified ? 'Verified' : 'Pending Check' }}</span>
                                    </td>
                                    <td>
                ...
                <div class="card sf-card mb-4">
                    <div class="card-body p-4">
                        ...
                        <form method="POST" action="{{ route('fassg.fixed-lists.import', $list) }}"
                            ...
                            @csrf
                            <div class="mb-3"><input type="file" name="file" class="form-control form-control-sm"
                                    accept=".csv,.txt" required></div>
                            <button type="submit" class="btn btn-outline-primary btn-sm w-100"><i
                                    class="bi bi-upload me-1"></i>Import File</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="card sf-card">
                    <div class="card-body p-4 text-center">
                        ...
                        <form method="POST" action="{{ route('fassg.fixed-lists.submit', $list) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-sf-gold w-100" @disabled($list->items->isEmpty())
                                onClick="if ({{ $list->items->where('is_sle_fhe_verified', true)->count() }} !== {{ $list->items->count() }}) { alert('Verify all students before forwarding this list to the sponsor.'); return false; } return confirm('All students are verified. Forward this list to the sponsor?');"><i
                                    class="bi bi-send me-1"></i>Submit List</button>
                        </form>
                    </div>
                </div>
            @endif
