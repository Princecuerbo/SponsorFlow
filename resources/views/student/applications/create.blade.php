...
@section('content')
    ...
    @if ($profile->hasActiveSponsorship())
        <div class="alert alert-danger border-0 shadow-sm rounded-3">
            ...
            You already have an active sponsorship. Only one active sponsorship is allowed at a time — you may re-apply once it expires.
    ...
    <form method="POST" action="{{ route('student.applications.store') }}" enctype="multipart/form-data" class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card sf-card mb-4">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center gap-2 mb-3">
                        <span class="badge bg-primary text-white rounded-circle" style="width:26px;height:26px;">1</span>
                        <h2 class="h6 sf-heading mb-0">Student &amp; SLE-FHE Profile</h2>
                    </div>
                    <p class="small text-secondary mb-3">Pulled from your verified profile. To change any of this, update your profile before applying.</p>

                    <div class="row g-3">
                        <div class="col-sm-6">
                            <label class="form-label small text-secondary">Student ID Number</label>
                            <input type="text" class="form-control sf-mono" value="{{ $profile->student_id_number }}" disabled>
                        </div>
                        <div class="col-sm-6">
            ...
            <div class="card sf-card mb-4">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center gap-2 mb-3">
                        <span class="badge bg-primary text-white rounded-circle" style="width:26px;height:26px;">2</span>
                        <h2 class="h6 sf-heading mb-0">Application Details</h2>
                    </div>

                    <div class="row g-3">
                        <div class="col-sm-4">
                            <label for="current_gpa" class="form-label small text-secondary">Current GPA <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" min="1" max="5" name="current_gpa" id="current_gpa"
                                value="{{ old('current_gpa', old('gpa_submitted')) }}" class="form-control @error('current_gpa') is-invalid @enderror" required>
                            @if ($program->min_gpa)
                                <div class="form-text">Program requires {{ number_format($program->min_gpa, 2) }} or better.</div>
                            @endif
                            @error('current_gpa') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="col-sm-8">
                            <label for="current_address" class="form-label small text-secondary">Current Address <span class="text-danger">*</span></label>
                        ...
                        <div class="col-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" name="is_rural_submitted" id="is_rural_submitted"
                                       value="1" {{ old('is_rural_submitted', $profile->is_rural) ? 'checked' : '' }}>
            ...
            <div class="card sf-card">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center gap-2 mb-3">
                        <span class="badge bg-primary text-white rounded-circle" style="width:26px;height:26px;">3</span>
                    ...
                    <div class="row g-3">
                        @foreach ($docs as $field => [$label, $icon])
                            <div class="col-md-4">
                                <label class="form-label small text-secondary d-block">{{ $label }} <span class="text-danger">*</span></label>
                                <label for="{{ $field }}" class="d-block border border-2 border-dashed rounded-3 text-center p-4 bg-light"
                                       style="cursor:pointer; border-style:dashed !important;">
                                    <i class="bi {{ $icon }} fs-3 text-secondary d-block mb-2"></i>
                                    <span class="small fw-semibold d-block">Click to upload</span>
                                    <span class="small text-secondary" data-filename-for="{{ $field }}">or drag file here</span>
                                    <input type="file" name="{{ $field }}" id="{{ $field }}" class="d-none"
        ...
        <div class="col-lg-4">
            <div class="card sf-card position-sticky" style="top:5.5rem;">
                <div class="card-body p-4">
                    ...
                    <button type="submit" class="btn btn-sf-gold w-100 mb-2">
                        <i class="bi bi-send me-1"></i> Submit Application
                    </button>
                    <a href="{{ route('student.programs.index') }}" class="btn btn-outline-secondary w-100">Cancel</a>

                    <div class="small text-secondary mt-3">
                        <i class="bi bi-shield-check me-1"></i>
                        Submitting locks this program's slot request into FASSG's verification queue. You'll be notified at each stage.
                    </div>
                </div>
            </div>
        </div>
