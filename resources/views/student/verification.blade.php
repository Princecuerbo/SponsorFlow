...
@section('content')
    ...
    <div class="row g-4">
        ...
        @method('PUT')
                        ...
                        <div class="col-md-6"><label class="form-label" for="academic_program_id">Academic Program / Course</label><select class="form-select" id="academic_program_id" name="academic_program_id" required><option value="" disabled @selected(!$profile?->academic_program_id)>-- Select Your Course/Program --</option>@foreach ($programs as $program)<option value="{{ $program->program_id }}" @selected((int) old('academic_program_id', $profile?->academic_program_id) === (int) $program->program_id)>{{ $program->code }} - {{ $program->name }}</option>@endforeach</select></div>
                        ...
                        <div class="col-12"><label class="form-label" for="address">Complete address</label><input class="form-control" id="address" name="address" value="{{ old('address', $profile?->full_address) }}" required></div>
