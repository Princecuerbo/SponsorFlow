...
@section('content')
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
        <div>
            ...
            <h1 class="h2 sf-heading mb-1">Forwarded Applicants & Lists</h1>
            <p class="text-secondary mb-0">Review FASSG-verified applications and sponsor-provided beneficiary lists.</p>
        </div>
        <div class="d-flex gap-2"><span class="badge text-bg-primary px-3 py-2">{{ $applicants->count() }}
                applications</span><span class="badge text-bg-light border text-dark px-3 py-2">{{ $fixedLists->count() }}
                fixed lists</span></div>
    </div>
    <div class="card sf-card mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('sponsor.applicants.index') }}" class="row g-2">
                ...
                <div class="col-md-2"><a href="{{ route('sponsor.applicants.index') }}"
    ...
    <section class="mb-5">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
                <h2 class="h5 sf-heading mb-1">Forwarded Applications</h2>
                <p class="small text-secondary mb-0">Individual students verified by FASSG and awaiting sponsor
                    confirmation.</p>
            </div><i class="bi bi-person-check text-primary fs-3"></i>
        </div>
        @if ($applicants->isEmpty())
            <div class="card sf-card">
                <div class="sf-empty-state"><i class="bi bi-person-lines-fill"></i>
                    <div class="fw-semibold">No forwarded applications</div>
                    <div class="small">FASSG-verified applications will appear here.</div>
                </div>
        </div>@else<div class="card sf-card">
                            ...
                            @foreach ($applicants as $application)
                                <tr>
                                    ...
                                    <td class="text-end pe-4"><a
                                            href="{{ route('sponsor.applicants.show', $application) }}"
                                            class="btn btn-sm btn-sf-navy"><i class="bi bi-eye me-1"></i>Review &amp;
                                            Confirm</a></td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <section>
        <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
                <h2 class="h5 sf-heading mb-1">Sponsor-Provided Fixed Lists</h2>
                <p class="small text-secondary mb-0">Batch lists forwarded by FASSG for sponsor confirmation.</p>
            </div><i class="bi bi-people text-primary fs-3"></i>
        </div>
        @if ($fixedLists->isEmpty())
            <div class="card sf-card">
                <div class="sf-empty-state"><i class="bi bi-inboxes"></i>
                    <div class="fw-semibold">No fixed lists forwarded yet</div>
                    <div class="small">FASSG lists will appear here once submitted.</div>
                </div>
        </div>@else<div class="card sf-card">
                <div class="table-responsive">
                    <table class="table sf-table mb-0">
                        ...
                        <tbody>
                            @foreach ($fixedLists as $list)
                                <tr>
                                    ...
                                    <td class="text-end pe-4"><a href="{{ route('sponsor.lists.show', $list) }}"
                                            class="btn btn-sm btn-outline-secondary">Review List <i
